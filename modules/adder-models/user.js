...
const userPreSaveUpdatedAtMiddleware = async function(next) {
    const logger = new ImmutableTagLogger("UserSchema.pre(\'save\')<" + this._id + ">");
...
UserSchema.methods.upgradeDriverAccount = async function(newPassword, hashedPassword) {
...
UserSchema.methods.comparePassword = async function(uid, password, role, hashedPassword, callback) {
    const logger = new ImmutableTagLogger("UserSchema.comparePassword()<" + uid + ">");
    if(role && role === 'driver') {
        if(hashedPassword) {
...
const clientPreSaveMiddleware = async function(next) {
    const logger = new ImmutableTagLogger("UserSchema.pre(\'save\').clientPreSaveMiddleware()<" + this._id + ">");
    ...
    if (typeof this.hash !== "undefined" && this.hash) {
        ...
        if (this.isModified("hash") || this.isNew) {
            ...
            logger.debug("this.hash was not marked as modified, or this.isNew was not set to true!");
        }
    }
