...
const isObjectIDValid = (id) => {
    if(typeof id === "undefined" || id === null) {
        ...
        return new Error(`Decoded JWT Payload contained an _id that failed regex test!`);
    } else {
        return true;
    }
};
...
app.post("/auth/register", async function (req, res) {
  ...
  try {
    ...
    console.log("spot 1.2 not returning await User.findOne({email: req.body.email});");
      try{
